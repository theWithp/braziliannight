  ...
  public ModelCreatureObj initModel (String path)
    {
      ...
      try
        {
          ...
          InputStream in = Minecraft.getMinecraft().getResourceManager().getResource(modelPartsLocation)
              .getInputStream();
      ...
      // Load Animations:
      ResourceLocation animationLocation = new ResourceLocation(BNConstants.MOD_ID,
          "models/" + path + "_animation.json");
      try
        {
          Gson gson = (new GsonBuilder()).setPrettyPrinting().disableHtmlEscaping().create();
          InputStream in = Minecraft.getMinecraft().getResourceManager().getResource(animationLocation)
              .getInputStream();
          BufferedReader reader = new BufferedReader(new InputStreamReader(in));
          try
            {
      ...
      for (ObjObject part : this.wavefrontParts)
        {
          ...
          this.wavefrontObject.renderGroup(part, new Vector4f(1, 1, 1, 1),
