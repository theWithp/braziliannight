  ...
  public void toMesh (Mesh mesh)
    {
      ...
      for (int i = 0; i < n; i++)
        {
          Vertex vertex = new Vertex(vertices.get(i), texCoords.get(i), normals.get(i), new Vector3f());
      ...
      for (int i = 0; i < indices.size(); i += 3)
        {
          ...
          Vector3f tangent = new Vector3f((float) (f * (deltaV2 * edge1.x - deltaV1 * edge2.x)),
              (float) (f * (deltaV2 * edge1.y - deltaV1 * edge2.y)),
              (float) (f * (deltaV2 * edge1.z - deltaV1 * edge2.z)));
